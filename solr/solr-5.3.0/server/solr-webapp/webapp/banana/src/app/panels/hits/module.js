...
if (self.list != undefined && self.list[0].query.length!==0){
    detectedLanguage = $scope.myFn(encodeURIComponent(self.list[0].query));
      ...
      var buildTerms = function(allLanguagesArr,lang){
          $.each(allLanguagesArr,function(lang){
                        $.ajax({
                            type: "GET",
                            async: false,
                            url: "https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20151213T022138Z.00f3f916f770556c.c7ea409c76ebb5c5b65502fb1f2cf13a84c85849&text=russia&lang=en-"+ lang,
      ...
      var combinedORTerm = '';
      if ($.inArray(detectedLanguage,allLanguagesArr)) {
        switch(detectedLanguage) {
            case 'en':
                qfToAppend = 'text_en^2+text_ru+text_de+text_fr';
                if (lang!=='en'){
                    buildTerms(allLanguagesArr,lang);  
                }
                break;
            case 'de':
          ...
          if (self.list[0].query !== '*'){
            combinedORTerm = enTerm+"OR"+ruTerm+"OR"+deTerm+"OR"+frTerm;    
          }
      }
//Added
       _.each($scope.panel.queries.ids, function(id) {
        var temp_q =  querySrv.getQuery(id) + fq + qf + facet + wt_json + rows_limit;
