...
if (self.list && self.list[0].query.length!==0){
    detectedLanguage = $scope.myFn(encodeURIComponent(self.list[0].query));
}
      
      //modifying query to include query terms all languages
      var ruTerm = '';
      var enTerm = '';
      var deTerm = '';
      var frTerm = '';
      var qfToAppend = '';
      var qf = '';
      var allLanguagesArr = ['en','de','fr','ru'];
      var buildTerms = function(allLanguagesArr,lang){
          var searchStr = querySrv.getQuery(0).substring(2);
          if (searchStr!=="*"){
            $.each(allLanguagesArr,function(index,lang1){
                        $.ajax({
                            type: "GET",
                            async: false,
                            url: "https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20151213T022138Z.00f3f916f770556c.c7ea409c76ebb5c5b65502fb1f2cf13a84c85849&text="+encodeURIComponent(searchStr)+"&lang="+detectedLanguage+"-"+ lang1,
                            success: function(data) {
                                switch(lang1) {
                                    case 'en': enTerm = data.text; break;
                                    case 'ru': ruTerm = data.text; break;
                                    case 'de': deTerm = data.text; break;
                                    case 'fr': frTerm = data.text; break;
                                    default: enTerm = data.text;
                                }
                            }
                        });
                    });    
      ...
      var combinedORTerm = '';
      if ($.inArray(detectedLanguage,allLanguagesArr)==0) {
        switch(detectedLanguage) {
            case 'en':
                qfToAppend = 'text_en^2+text_ru+text_de+text_fr';
                // if (detectedLanguage!=='en'){
                    buildTerms(allLanguagesArr,detectedLanguage);  
                // }
                break;
            case 'de':
                qfToAppend = 'text_en+text_ru+text_de^2+text_fr';
                // if (detectedLanguage!=='de'){
                    buildTerms(allLanguagesArr,detectedLanguage);  
                // }
                break;
            case 'ru':
                qfToAppend = 'text_en+text_ru^2+text_de+text_fr';
                // if (detectedLanguage!=='ru'){
                    buildTerms(allLanguagesArr,detectedLanguage);  
                // }
            break;
            case 'fr':
                qfToAppend = 'text_en+text_ru+text_de+text_fr^2';
                // if (detectedLanguage!=='fr'){
                    buildTerms(allLanguagesArr,detectedLanguage);  
                // }
            break;
            default:
                qfToAppend = 'text_en+text_ru+text_de+text_fr';
        }
          qf = '&defType=dismax&qf=' + qfToAppend;   
          if (querySrv.getQuery(0).substring(2)!=="*"){
            combinedORTerm = enTerm+" "+ruTerm+" "+deTerm+" "+frTerm;    
          }
          if (combinedORTerm!=""){
              combinedORTerm = " "+combinedORTerm;
          }
      }
//Added
        //var query = $scope.panel.searchQuery == null ? querySrv.getQuery(0) : 'q=' + $scope.panel.searchQuery
        if (querySrv.getORquery().substring(2)=="*"){
          //  $scope.panel.queries.basic_query = querySrv.getORquery()"q=*";
         // $scope.panel.queries.basic_query = querySrv.getORquery() + fq + qf + facet + facet_fields + sorting;
         $scope.panel.queries.basic_query = querySrv.getORquery() + fq + facet + facet_fields + sorting;
        }
        else {
            $scope.panel.queries.basic_query = querySrv.getORquery() +combinedORTerm + fq + qf + facet + facet_fields + sorting;    
        }
